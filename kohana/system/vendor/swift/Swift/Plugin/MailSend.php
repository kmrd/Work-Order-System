...
class Swift_Plugin_MailSend implements Swift_Events_SendListener, Swift_Events_BeforeSendListener
{
  ...
  public function sendPerformed(Swift_Events_SendEvent $e)
  {
	...
	$params = $this->getAdditionalParams(); // david patch
	$params['returnPath'] = $this->returnPath; // david patch
	
//    $this->doMail($to, $subject_enc, $message_body, $message->headers, sprintf($this->getAdditionalParams(), $this->returnPath));
    $this->doMail($to, $subject_enc, $message_body, $message->headers, $params);
  ...
  public function doMail($to, $subject, $message, $headers, $params)
  {
	...
	@ini_set('SMTP',$params['hostname']);
	@ini_set('smtp_port',$params['port']);
	@ini_set('',$params['username']);
	@ini_set('',$params['password']);
	@ini_set('',$params['encryption']);
    
/*    $headers = $headers->build();
    $params = "-host smtp.googlemail.com -fmailer@studionostalgia.ca";
